...
class Home extends Component{
  ...
  listNews(){
    const token = localStorage.getItem('token');
  	axios.get('http://backdziennik.nilow13.usermd.net/api/news/index/' + this.state.offset + '/' + this.state.perPage + '?token='+ token)
      .then(res => {
...
eventAdding(newTitle, newTeamsId, newTrainersId, newStart, newEnd){
  ...
  this.setState({offset: offset},()=>{this.listNews()});
